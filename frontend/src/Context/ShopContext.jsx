...
const ShopContextProvider = (props) => {
    ...
    useEffect(() => {
        fetch('https://e-commerce-backend-37cn.onrender.com/allproducts')
            .then(response => response.json())
            .then(data => setAll_Product(data))
            .catch(error => console.error('Error fetching products:', error));

        if (localStorage.getItem('auth-token')) {
            fetch('https://e-commerce-backend-37cn.onrender.com/getcart', {
                method: 'POST',
                headers: {
    ...
    const addToCart = (itemId) => {
        ...
        if (localStorage.getItem('auth-token')) {
            fetch('https://e-commerce-backend-37cn.onrender.com/addtocart', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'auth-token': `${localStorage.getItem('auth-token')}`,
    ...
    const removeFromCart = (itemId) => {
        ...
        if (localStorage.getItem('auth-token')) {
            fetch('https://e-commerce-backend-37cn.onrender.com/removefromcart', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'auth-token': `${localStorage.getItem('auth-token')}`,
